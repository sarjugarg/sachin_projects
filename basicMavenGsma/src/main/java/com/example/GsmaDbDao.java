...
class GsmaDbDao {
        ...
        try {
            ...
            if (!product.getBrandName().equals("NA")) {
                ...
                try {
                    ...
                    stmt.executeQuery(" update device_usage_db set FEATURE_NAME = 'V' where tac  =   '" + product.getDeviceId() + "' ");

                } catch (Exception e) {
                    logger.warn(" NOt updated in dev_usbDb " + e);
                }
            } else {
    ...
    void invalidGsmaDb(String deviceId, Connection conn) {
        ...
        try {
            ...
            try {
                stmt.executeQuery(" update device_usage_db set FEATURE_NAME = 'I' where tac  =   '" + deviceId + "' ");
            } catch (Exception e) {
                logger.warn(" NOt updated in dev_usbDb " + e);
            }
